...
describe('UserService', () => {
  ...
  const userArray = [oneApplicant, twoApplicant]
        ...
        ApplicantsService,
        {
          ...
          useValue: {
            applicants: {
              ...
              update: vi.fn().mockResolvedValue(updatedApplicant),
  ...
  describe('createOne', () => {
    it('should successfully add a user', async () => {
      await expect(service.create(oneApplicant)).resolves.toEqual(oneApplicant)
      expect(prisma.applicants.create).toBeCalledTimes(1)
    })
  })

  describe('findAll', () => {
    it('should return an array of applicants', async () => {
      const applicants = await service.findAll()
      expect(applicants).toEqual(userArray)
    })
  })

  describe('findOne', () => {
    it('should get a single user', async () => {
      await expect(service.findOne(1)).resolves.toEqual(oneApplicant)
    })
  })

  describe('update', () => {
    it('should call the update method', async () => {
      const user = await service.update(1, updateApplicant)
      expect(user).toEqual(updateApplicant)
      expect(prisma.applicants.update).toBeCalledTimes(1)
    })
  })
  ...
  describe('remove', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = vi
        .spyOn(prisma.applicants, 'delete')
  ...
  describe('searchApplicants', () => {
    it('should return a list of applicants with pagination and filtering', async () => {
      const page = 1
      const limit = 10
      ...
      vi.spyOn(prisma.applicants, 'findMany').mockResolvedValue([])
      vi.spyOn(prisma.applicants, 'count').mockResolvedValue(0)
    ...
    it('given no page should return a list of applicants with pagination and filtering with default page 1', async () => {
      const limit = 10
      const sortObject: Prisma.SortOrder = 'asc'
      const sort: any = `[{ "name": "${sortObject}" }]`
      const filter: any = '[{ "name": { "equals": "Peter" } }]'
    ...
    it('given no limit should return a list of applicants with pagination and filtering with default limit 10', async () => {
      const page = 1
      const sortObject: Prisma.SortOrder = 'asc'
      const sort: any = `[{ "name": "${sortObject}" }]`
      const filter: any = '[{ "name": { "equals": "Peter" } }]'
    ...
    it('given  limit greater than 200 should return a list of applicants with pagination and filtering with default limit 10', async () => {
      const page = 1
      const limit = 201
      const sortObject: Prisma.SortOrder = 'asc'
      const sort: any = `[{ "name": "${sortObject}" }]`
