...
import { IncompleteRecordsDialog } from '../IncompleteRecordsDialog'
...
export type EligibilityListMode = 'standard' | 'dq'
...
export default function EligibilityListPage({
  ...
  // Pagination states for batch history / audit trail
  ...
  const handleAddToBatch = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        await getAllBatches()
        setBatches(await getAllBatches())
        onBatchesChanged?.()
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (mode !== 'standard' || selectedChild !== null || rememberedChildId === null) return
  ...
  }, [mode, selectedChild, rememberedChildId, filteredData])
  ...
  const canDqModifySelected = useMemo(() => {
    if (mode !== 'dq' || selected.length !== 1) return false
    const cached = selectedRecordsCache.get(selected[0])
    if (!cached) return false
    return canDqModifyRecord(mode === 'dq', selected.length, cached.csaStatusRaw)
  }, [mode, selected, selectedRecordsCache])
